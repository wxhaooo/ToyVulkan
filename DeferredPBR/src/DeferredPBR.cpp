...
void DeferredPBR::BakingIrradianceCubeMap()
{
	...
	// Offfscreen framebuffer
	{
		...
		VkWriteDescriptorSet writeDescriptorSet = vks::initializers::WriteDescriptorSet(descriptorset, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 0, &environmentCubeMap->descriptor);
		...
		const std::vector<VkVertexInputAttributeDescription> vertexInputAttributes = {
			...
			vks::initializers::VertexInputAttributeDescription(0, 2, VK_FORMAT_R32G32_SFLOAT, offsetof(vks::geometry::VulkanGLTFModel::Vertex, uv)),	// Location 2: Texture coordinates
		};
		...
		shaderStages[0] = LoadShader(vks::helper::GetShaderBasePath() + "deferred/filtercube.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = LoadShader(vks::helper::GetShaderBasePath() + "deferred/irradiancecube.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
		VkPipeline pipeline;
